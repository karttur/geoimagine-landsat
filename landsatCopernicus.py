...
class LandsatComposition:
    '''
    class for landsat compositions
    '''
...
class ProcessLandsat:
    '''class for landsat specific processing'''   
    def __init__(self, process, session, verbose):
        ...
        if self.process.proc.processid == 'extractlandsattilecoords':
            self._ExtractLandsatTileCoords()
        else:
    ...
    def _ExtractLandsatTileCoords(self):
        '''Extracts landsat scene corners from MGRS polygons
        '''
    ...
    def _ExtractVectorCoords(self,srcFPN):
        ...
        if self.process.params.wrs2:
            wrs = 2
            refsize = 30
        else:
            wrs = 1
            refsize = 60
        dirField = self.process.params.dirfield
        #print ('dirfiled',dirField)
        pathField = self.process.params.pathfield
        #print ('pathfiled',pathField)
        rowField = self.process.params.rowfield
        #print ('drowfiled',rowField)
        ...
        for feature in srcLayer.layer:
            ...
            r = feature.GetField(rowField)
            ldir = feature.GetField(dirField)

            #utmzone = int(mgrs[0:2])
            #latzone = mgrs[2:3]
            printstr = 'Extracting landsat p r  %(p)d %(r)d' %{'p':p, 'r':r}
            print (printstr)
            '''
            if p == 75 and r == 48:
                print (printstr)
            ...
            if srcLayer.geomtype.lower() != 'polygon':
            ...
            elif geom.shapelyGeom.geom_type == 'MultiPolygon':
                pass
            else:
                ...
                #print (geom.shapelyGeom.geom_type)
                coords = list(geom.shapelyGeom.exterior.coords)
                print ('ini coords',len(coords),coords)
                ...
                if len(coords) > 5:
                    filt = 0.0001
                    while True:
                        s = geom.shapelyGeom.simplify(filt, preserve_topology=False)
                        filt *= 10
                        coords = list(s.exterior.coords)
                        print ('len', filt, len(coords))
                        if len(coords) <= 5 or filt >= 1:
                            break
                        break
                    #print ('s',s)
                    coords = list(s.exterior.coords)
                ...
                #print (len(coords))
                if len(coords) > 5:
                    #Check om det går att hitta via w,es,n,
                    vals = 3
                    while True:
                        popL = []
                        keepL = []
                        for i,c in enumerate(coords):
                            if round(c[1],vals) == round(north,vals) or round(c[1],vals) == round(south,vals) or round(c[0],vals) == round(east,vals) or round(c[0],vals) == round(west,vals):
                                keepL.append(i)
                            else:
                                popL.append(i)
                                
                        vals += 1
                        if len(keepL) == 4 or len(keepL) == 5 or vals > 10:
                            break
                        
                    if len(keepL) == 4 or len(keepL) == 5:
                        popL.reverse()
                        for pp in popL:
                            coords.pop(pp)
                        #print ('coords',coords)
                    else:
                        if len(keepL) == 6 and round(coords[0][1],4) == round(coords[5][1],4):
                            #if no two of the ithers
                            morenr = 0
                            for c in range(1,5):
                                if round(coords[c][1],4) == round(coords[0][1],4):
                                    morenr += 1
                            if morenr == 2:
                                coords.pop(5)
                                coords.pop(0)
                ...
                if len(coords) != 4:
                    ...
                    query = {'wrs':wrs,'path':p,'row':r, 'dir':ldir,
